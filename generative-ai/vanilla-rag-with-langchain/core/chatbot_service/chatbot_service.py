...
class ChatbotService(BaseGenerativeService):
    ...
    def load_config(self, context):
        ...
        config.update({"local_model_path": context.artifacts.get("models", "")})
    ...
    def load_local_model(self, context):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"LlamaCpp model initialized successfully with context window of {context_window} tokens"
                )
    ...
    def load_local_hf_model(self, context):
        ...
        logger.info(
            f"Using the local Deep Seek model downloaded from HuggingFace with context window of {context_window} tokens."
        )
    ...
    def load_cloud_hf_model(self, context):
        ...
        logger.info(
            f"Using the cloud Mistral model on HuggingFace with context window of {context_window} tokens."
        )
    ...
    def load_vector_database(self):
        ...
        if not self.docs_path or not os.path.exists(self.docs_path):
            raise FileNotFoundError(
                f"The documents directory was not found at: {self.docs_path}"
            )

        pdf_path = os.path.join(self.docs_path, "AIStudioDoc.pdf")
        if not os.path.exists(pdf_path):
            raise FileNotFoundError(
                f"The file 'AIStudioDoc.pdf' was not found at: {pdf_path}"
            )
        ...
        try:
            ...
            logger.info(
                "Using embedding model initialized during service initialization."
            )

            # Create vector database
            logger.info("Creating vector database...")
            self.vectordb = Chroma.from_documents(
                documents=splits, embedding=self.embedding
            )
    ...
    def load_chain(self) -> None:
        ...
        self.chain = (
            input_normalizer
            | RunnableMap(
                {"context": retriever_runnable | format_docs_r, "query": extract_input}
            )
            | self.prompt
            | self.llm
            | StrOutputParser()
    ...
    def add_pdf(self, base64_pdf):
        ...
        try:
            ...
            vectordb = Chroma.from_documents(
                documents=new_splits, embedding=self.embedding
            )
    ...
    def inference(self, context, user_query):
        ...
        try:
            ...
            response = self.chain.invoke({"input": user_query, "output": ""})

            # Get relevant documents using context-aware retrieval
            relevant_docs = dynamic_retriever(
                user_query, self.vectordb, context_window=context_window
            )
    ...
    def log_model(
        ...
        # Prepare artifacts
        artifacts = {"config": config_path, "docs": docs_path}
