...
class VectorStoreWriter:
    def __init__(
        self, collection_name="my_collection", verbose=False, embedding_model=None
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"ChromaDB client initialized with persistent storage at {persist_dir}"
            )
        except Exception as e:
            logger.warning(
                f"Error creating persistent client: {str(e)}, trying in-memory client"
            )
        ...
        try:
            ...
            logger.info(
                f"ChromaDB collection '{collection_name}' initialized with persistent storage."
            )
            ...
            logger.info(
                f"ChromaDB collection '{collection_name}' initialized with fallback method."
            )
    ...
    def upsert_dataframe(self, df):
        ...
        for i, row in df.iterrows():
            ...
            if embedding is None or (
                ...
                and (len(embedding) == 0 or any(e is None for e in embedding))
            ):
                logger.warning(
                    f"Invalid embedding for ID {row['ids']} - replacing with zeros"
                )
        ...
        if self.verbose:
            for i in range(len(valid_ids)):
                ...
                logger.info(
                    f"[UPDATING] Embedding: {valid_embeddings[i][:5]}...\n"
        ...
        try:
            ...
            try:
                logger.warning(
                    "Attempting to upsert without embeddings (ChromaDB will generate them)"
                )
                self.collection.upsert(
                    documents=valid_documents, ids=valid_ids, metadatas=valid_metadatas
                )
