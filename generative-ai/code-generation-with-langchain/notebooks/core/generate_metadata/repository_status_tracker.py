...
class RepositoryStatusTracker:
    ...
    def update_progress(
        self, repo_id: str, files_processed: int, total_files: int
    ) -> Dict[str, Any]:
        """
        ...
        with self.lock:
            ...
            self.logger.debug(
                f"Updated progress for {repo_id}: {status['progress']}% ({files_processed}/{total_files})"
            )
            return status
    ...
    def complete_processing(self, repo_id: str, context: Any) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            status["processing_duration"] = (
                status["completion_time"] - status["start_time"]
            )

            self.logger.info(
                f"Completed processing repository {repo_id} in {status['processing_duration']:.2f}s"
            )
            return status
