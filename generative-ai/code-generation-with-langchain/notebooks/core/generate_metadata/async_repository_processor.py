...
class AsyncRepositoryProcessor:
    ...
    def get_repository_status(
        ...
        public_status = {
            ...
            if k not in ["start_time", "last_update_time"]
        }
        return public_status

    def process_repository_async(
        self,
        repo_url: str,
        branch: str = "main",
        ...
        with self._lock:
            ...
            if (
                current_status
                and current_status.get("status") == ProcessingStatus.COMPLETE.value
            ...
            if (
                current_status
                and current_status.get("status") == ProcessingStatus.PROCESSING.value
                and not force_refresh
            ):
            ...
            if force_refresh and current_status:
                ...
                if repo_id in self.active_futures:
                    ...
                    if future.running():
                        self.logger.warning(
                            f"Canceling active processing for repository: {repo_url}"
                        )
                        future.cancel()
            ...
            self.logger.info(
                f"Starting background processing for repository: {repo_url}"
            )
    ...
    def _process_repository_task(
        ...
        try:
            ...
            extractor = self.repository_extractor_class(
                repo_url=repo_url, **extraction_params
            )
            ...
            self.status_tracker.update_progress(
                repo_id=repo_id, files_processed=0, total_files=len(extracted_data)
            )
            ...
            self.logger.info(
                f"Completed repository processing in {processing_time:.2f}s: {repo_url}"
            )
    ...
    def wait_for_completion(
        self, repo_url: str, branch: str = "main", timeout: Optional[float] = None
    ) -> Dict[str, Any]:
        """
