# 🤖 Vanilla RAG with LangChain and Opik Evaluation
...
This project is an AI-powered vanilla **RAG (Retrieval-Augmented Generation)** chatbot built using **LangChain** and **Opik** for model evaluation, protection, and observability. It leverages the **Z by HP AI Studio Local GenAI image** and the **llama3.1-8b-instruct-Q8_0** model to generate contextual and document-grounded answers to user queries about **Z by HP AI Studio**.
...
- Add your API keys to the `secrets.yaml` file located in the `configs` folder:
  ...
  - `OPIK_API_KEY`: Required for Opik evaluation.
  - `OPENAI_API_KEY`: Optional, used if you want to use OpenAI models for evaluation rather than the local LLaMA model.
...
- Integrate Opik evaluation, LLM as a Judge
