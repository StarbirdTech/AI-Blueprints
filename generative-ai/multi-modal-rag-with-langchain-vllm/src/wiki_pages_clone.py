...
def run_wiki_clone(pat: str, org: str, project: str, wiki_id: str, output_dir: Path):
    ...
    if not pat:
        raise ValueError(
            "Azure DevOps PAT cannot be empty. Please set the AZURE_DEVOPS_PAT environment variable."
        )
...
def orchestrate_wiki_clone(pat: Optional[str], config: dict, output_dir: Path):
    ...
    if not all([pat, org, project, wiki_id]):
        logger.warning(
            "SKIPPING ADO WIKI CLONE: Not all required credentials or configurations were found."
        )
        ...
        if not wiki_metadata_file.is_file() or not image_dir.is_dir():
            ...
            logger.warning(
                f"Found existing data in '{output_dir}'. The notebook will proceed using this data."
            )
    else:
        ...
        try:
            ...
            logger.error(
                "Please check your ADO PAT, organization, project, and wiki ID details."
            )
            raise
