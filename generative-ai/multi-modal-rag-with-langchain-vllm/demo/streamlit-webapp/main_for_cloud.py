...
def call_model_api(api_url: str, query: str, config_payload: dict) -> dict:
    ...
    record = {"query": query, "payload": json.dumps(config_payload)}
    ...
    try:
        ...
        response = requests.post(
            api_url, json=payload, headers=headers, verify=False, timeout=300
        )
...
def main():
    ...
    with st.sidebar:
        ...
        ado_org = st.text_input(
            "ADO Organization", key="ado_org", placeholder="e.g., MyCompany"
        )
        ado_project = st.text_input(
            "ADO Project", key="ado_project", placeholder="e.g., MyProject"
        )
        ado_wiki = st.text_input(
            "ADO Wiki Name", key="ado_wiki", placeholder="e.g., MyProject.wiki"
        )
        ado_pat = st.text_input(
            "ADO PAT",
            key="ado_pat",
            type="password",
            help="Your Personal Access Token for ADO.",
        )
    ...
    for message in st.session_state.messages:
        ...
        with st.chat_message(message["role"], avatar=avatar):
            ...
            if message["role"] == "assistant" and message.get("data"):
                ...
                c1.metric(
                    "Total Pipeline Time",
                    f"{data.get('total_pipeline_time_seconds', 0):.2f} s",
                )
    ...
    if prompt := st.chat_input("Ask a question about the wiki..."):
        ...
        with st.spinner(
            "🧠 Fetching database, thinking, and cleaning up... This may take a moment."
        ):
            config_payload = {
                "config": {
        ...
        assistant_message = {"role": "assistant", "content": "", "data": None}
        if response.get("success"):
            assistant_message["data"] = response.get("data", {})
            assistant_message["content"] = assistant_message["data"].get(
