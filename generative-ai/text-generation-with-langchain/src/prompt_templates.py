...
"""
        ...
        return f"""system
You are a helpful assistant that provides accurate and concise responses for scientific paper analysis and text generation.
...
def format_scientific_paper_analysis_prompt(model_source: str = "local") -> PromptTemplate:
    """
    ...
    """
    formatted_template = get_prompt_template_for_model_source(model_source, SCIENTIFIC_PAPER_ANALYSIS_TEMPLATE)
...
def format_text_generation_prompt(model_source: str = "local") -> PromptTemplate:
    ...
    """
    formatted_template = get_prompt_template_for_model_source(model_source, TEXT_GENERATION_TEMPLATE)
    return PromptTemplate.from_template(formatted_template)
