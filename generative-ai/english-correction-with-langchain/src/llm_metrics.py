    ...
    def get_client(cls, model_path: Optional[str] = None) -> Llama:
        ...
        if cls._client is None:
            if model_path is None:
                raise ValueError(
                    "Must provide model_path to initialize local LLaMA judge."
                )
...
Answer:"""
        ...
        try:
            ...
            try:
                ...
                with open(
                    "llm_eval_logs/local_llama_responses.txt", "a", encoding="utf-8"
                ) as f:
            ...
            logger.error(f"[LLaMA judge error]: {e}")
            scores.append(5.0)
...
grammar_score_metric = make_metric(
    eval_fn=grammar_score_eval_fn, greater_is_better=True, name="grammar_score"
)
