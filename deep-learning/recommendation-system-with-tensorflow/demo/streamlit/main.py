...
# --- Header ---
st.markdown(
    "<h1 style='text-align: center; color: #2C3E50;'>🎥 Movie Recommendation Agent</h1>",
    unsafe_allow_html=True,
)

st.markdown(
    "<h4 style='text-align: center; color: #555;'> Have a movie recommendation based on your movie rating.</h3>",
    unsafe_allow_html=True,
)
...
# 2 ▸ Main – data input
# ─────────────────────────────────────────────────────────────
movie_id = st.number_input("Enter a movie ID:", min_value=0)
rating = st.number_input("Enter a rating", min_value=0.0, max_value=5.0, step=0.5)
