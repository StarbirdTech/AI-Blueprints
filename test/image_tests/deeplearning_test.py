...
class TestBertNotebook(unittest.TestCase):
    ...
    def test_torch_installation(self):
        try:
            import torch
            self.assertTrue(torch.__version__ is not None, "Torch is not installed or version is not accessible.")
            self.assertTrue(hasattr(torch, 'cuda'), "Torch does not have CUDA support.")
            self.assertTrue(torch.cuda.is_available(), "CUDA is not available with the installed Torch version.")
        except ImportError:
