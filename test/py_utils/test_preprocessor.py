...
class TestPreprocessor:
    ...
    def preprocess_test_file(self, base_folder, file_name):
        ...
        if "notebook" in file_metadata:
            ...
            import_line = (
                f"from {self.fnu.get_module(notebook_path)} import {class_name} \n"
            )
            lines.insert(next_line, import_line)
            real_notebook_path = os.path.realpath(
                os.path.join(base_folder, os.path.dirname(file_name), notebook_path)
            )
    ...
    def copy_requirements(self):
        ...
        for file_name in self.metadata["code"]:
            requirements_file = self.metadata["code"][file_name].get(
                "requirements", None
            )
            if requirements_file is not None:
                requirements_file = os.path.join(
                    self.base_folder, os.path.dirname(file_name), requirements_file
                )
                # requirements_file = self.fnu.fix_filename(requirements_file, extension=".txt")
                target_requirements_file = os.path.join(
                    self.target_folder, f"{file_name}t_requirements.txt"
                )
                shutil.copyfile(
                    requirements_file,
                    self.fnu.fix_filename(target_requirements_file, extension=".txt"),
                )
