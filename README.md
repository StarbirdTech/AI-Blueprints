...
7. **CUDA OOM on Ubuntu despite free VRAM (VRAM fragmentation on display GPU)**
   Linux desktop apps/compositor fragment VRAM, so a large contiguous cudaMalloc fails while Windows succeeds. Fix: close GPU-accelerated apps or use a non-display GPU; for llama.cpp set low_vram=False, n_ubatch=16–32, n_batch=32–64 (keep n_gpu_layers=-1 for faster inference).
